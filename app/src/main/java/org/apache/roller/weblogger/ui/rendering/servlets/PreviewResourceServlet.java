...
public class PreviewResourceServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        WeblogPreviewResourceRequest resourceRequest = createResourceRequest(request);
        ...
        if (weblog == null) {
            log.debug("error creating weblog resource request");
            response.sendError(HttpServletResponse.SC_NOT_FOUND);
            return;
        }
        ...
        Resource resource = findResource(resourceRequest, weblog);
        if (resource == null) {
            log.debug("Unable to get resource");
            response.sendError(HttpServletResponse.SC_NOT_FOUND);
            return;
        }
    ...
    private WeblogPreviewResourceRequest createResourceRequest(HttpServletRequest request) {
        try {
            return new WeblogPreviewResourceRequest(request);
        } catch (Exception e) {
    ...
    private Resource findResource(WeblogPreviewResourceRequest resourceRequest, Weblog weblog) {
        ...
        try {
            ...
            log.error("Error getting theme", ex);
            return null;
        }
