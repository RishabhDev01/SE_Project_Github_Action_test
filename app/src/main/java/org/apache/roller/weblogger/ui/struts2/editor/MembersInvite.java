...
public class MembersInvite extends UIAction {
    ...
    public String save() {
        ...
        try {
            User user = getUserManager().getUserByUserName(getUserName());
            if (user == null) {
                addError("inviteMember.error.userNotFound");
                return INPUT;
            }

            if (hasExistingPermissionOrInvitation(user)) {
                return INPUT;
            }
            ...
            log.error("Error processing weblog invitation", ex);
    ...
    private boolean hasExistingPermissionOrInvitation(User user) {
        try {
            WeblogPermission perm = getUserManager().getWeblogPermissionIncludingPending(getActionWeblog(), user);
    ...
    private void sendInvitation(User user) throws WebloggerException {
        getUserManager().grantWeblogPermissionPending(getActionWeblog(), user, Collections.singletonList(getPermissionString()));
