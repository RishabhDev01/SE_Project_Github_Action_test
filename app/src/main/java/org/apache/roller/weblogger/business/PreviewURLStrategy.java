...
public class PreviewURLStrategy extends MultiWeblogURLStrategy {
    ...
    public String getWeblogURL(Weblog weblog, String locale, boolean absolute) {
        return buildWeblogURL(weblog, locale, absolute, Collections.emptyMap());
    }
    ...
    public String getWeblogEntryURL(Weblog weblog, String locale, String previewAnchor, boolean absolute) {
        ...
        return buildWeblogURL(weblog, locale, absolute, params);
    }
    ...
    public String getWeblogPageURL(Weblog weblog, String locale, String pageLink, String entryAnchor, String category, String dateString, List<String> tags, int pageNum, boolean absolute) {
        ...
        return buildWeblogPageURL(weblog, locale, pageLink, absolute, params);
    }
    ...
    private String buildWeblogURL(Weblog weblog, String locale, boolean absolute, Map<String, String> params) {
        if (weblog == null) {
            return null;
        }
        StringBuilder url = new StringBuilder(URL_BUFFER_SIZE);
        url.append(getBaseURL(absolute)).append(PREVIEW_URL_SEGMENT).append(weblog.getHandle()).append('/');
        if (locale != null) {
    ...
    private String buildWeblogPageURL(Weblog weblog, String locale, String pageLink, boolean absolute, Map<String, String> params) {
        if (weblog == null) {
            return null;
        }
        StringBuilder url = new StringBuilder(URL_BUFFER_SIZE);
    ...
    private String buildResourceURL(Weblog weblog, String filePath, boolean absolute, Map<String, String> params) {
        ...
        StringBuilder url = new StringBuilder(URL_BUFFER_SIZE);
        url.append(getBaseURL(absolute)).append("/roller-ui/authoring/previewresource/").append(weblog.getHandle()).append('/');
    ...
    private String getBaseURL(boolean absolute) {
        return absolute ? WebloggerRuntimeConfig.getAbsoluteContextURL() : WebloggerRuntimeConfig.getRelativeContextURL();
    }
}