...
public class GlobalConfig extends UIAction implements HttpParametersAware, ServletRequestAware {
    ...
    private void loadProperties() throws WebloggerException {
        PropertiesManager mgr = WebloggerFactory.getWeblogger().getPropertiesManager();
        setProperties(mgr.getProperties());
    }

    private void loadWeblogs() throws WebloggerException {
    ...
    private void validateAndSaveProperties() throws WebloggerException {
        for (String propName : getProperties().keySet()) {
            ...
            validateProperty(updProp, incomingProp, propertyDef);
        }
    ...
    private void validateProperty(RuntimeConfigProperty updProp, String incomingProp, PropertyDef propertyDef) {
        if (propertyDef.getType().equals("boolean")) {
            validateBooleanProperty(updProp, incomingProp, propertyDef);
        } else if (incomingProp != null && propertyDef.getType().equals("integer")) {
            validateIntegerProperty(updProp, incomingProp, propertyDef);
        } else if (incomingProp != null && propertyDef.getType().equals("float")) {
            validateFloatProperty(updProp, incomingProp, propertyDef);
        } else if (incomingProp != null) {
            updProp.setValue(incomingProp.trim());
        } else if (propertyDef.getName().equals("users.comments.plugins")) {
            // not a problem
        } else {
            addError("ConfigForm.invalidProperty", propertyDef.getName());
        }
    }

    private void validateBooleanProperty(RuntimeConfigProperty updProp, String incomingProp, PropertyDef propertyDef) {
        try {
            ...
            addError("ConfigForm.invalidBooleanProperty", Arrays.asList(propDesc, propertyDef.getName()));
        }
    }

    private void validateIntegerProperty(RuntimeConfigProperty updProp, String incomingProp, PropertyDef propertyDef) {
        try {
            Integer.parseInt(incomingProp);
            updProp.setValue(incomingProp);
        } catch (NumberFormatException nfe) {
            String propDesc = bundle.getString(propertyDef.getKey());
            addError("ConfigForm.invalidIntegerProperty", Arrays.asList(propDesc, propertyDef.getName()));
        }
    }

    private void validateFloatProperty(RuntimeConfigProperty updProp, String incomingProp, PropertyDef propertyDef) {
        try {
            Float.parseFloat(incomingProp);
            updProp.setValue(incomingProp);
        } catch (NumberFormatException nfe) {
            String propDesc = bundle.getString(propertyDef.getKey());
            addError("ConfigForm.invalidFloatProperty", Arrays.asList(propDesc, propertyDef.getName()));
        }
    }
