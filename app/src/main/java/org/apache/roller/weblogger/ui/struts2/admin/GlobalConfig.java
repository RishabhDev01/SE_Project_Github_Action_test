...
public class GlobalConfig extends UIAction implements HttpParametersAware, ServletRequestAware {
    ...
    private void validateProperties() {
        for (String propName : getProperties().keySet()) {
            ...
            validateProperty(updProp, propertyDef, incomingProp);
        }
    }

    private void validateProperty(RuntimeConfigProperty updProp, PropertyDef propertyDef, String incomingProp) {
        if (propertyDef.getType().equals("boolean")) {
            validateBooleanProperty(updProp, propertyDef, incomingProp);
        } else if (propertyDef.getType().equals("integer")) {
            validateIntegerProperty(updProp, propertyDef, incomingProp);
        } else if (propertyDef.getType().equals("float")) {
            validateFloatProperty(updProp, propertyDef, incomingProp);
        } else {
            validateStringProperty(updProp, propertyDef, incomingProp);
        }
    }

    private void validateBooleanProperty(RuntimeConfigProperty updProp, PropertyDef propertyDef, String incomingProp) {
        try {
    ...
    private void validateIntegerProperty(RuntimeConfigProperty updProp, PropertyDef propertyDef, String incomingProp) {
        if (incomingProp != null) {
            try {
    ...
    private void validateFloatProperty(RuntimeConfigProperty updProp, PropertyDef propertyDef, String incomingProp) {
        if (incomingProp != null) {
            try {
    ...
    private void validateStringProperty(RuntimeConfigProperty updProp, PropertyDef propertyDef, String incomingProp) {
        if (incomingProp != null) {
    ...
    private void saveConfigProperties() throws WebloggerException {
        PropertiesManager mgr = WebloggerFactory.getWeblogger().getPropertiesManager();
