...
public class GlobalConfig extends UIAction implements HttpParametersAware, ServletRequestAware {
    ...
    private void saveProperties() throws WebloggerException {
        for (String propName : getProperties().keySet()) {
            ...
            if (propertyDef.getType().equals("boolean")) {
                saveBooleanProperty(updProp, incomingProp);
            } else if (incomingProp != null && propertyDef.getType().equals("integer")) {
                saveIntegerProperty(updProp, incomingProp);
            } else if (incomingProp != null && propertyDef.getType().equals("float")) {
                saveFloatProperty(updProp, incomingProp);
        ...
        if (hasActionErrors()) {
            throw new WebloggerException("Action errors");
        }
    ...
    private void saveBooleanProperty(RuntimeConfigProperty updProp, String incomingProp) {
        try {
            ...
            String propDesc = bundle.getString(globalConfigDef.getPropertyDef(updProp.getName()).getKey());
            addError("ConfigForm.invalidBooleanProperty", Arrays.asList(propDesc, updProp.getName()));
        }
    }

    private void saveIntegerProperty(RuntimeConfigProperty updProp, String incomingProp) {
        try {
    ...
    private void saveFloatProperty(RuntimeConfigProperty updProp, String incomingProp) {
        try {
