...
public class SearchServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            WeblogSearchRequest searchRequest = parseSearchRequest(request);
            ...
            Map<String, Object> model = createRenderingModel(pageRequest, searchRequest);

            Renderer renderer = getRenderer(model, searchRequest);
            CachedContent rendererOutput = renderContent(model, renderer);

            flushRenderedContent(response, rendererOutput);
    ...
    private WeblogSearchRequest parseSearchRequest(HttpServletRequest request) {
        try {
            return new WeblogSearchRequest(request);
        } catch (Exception e) {
            log.debug("Error creating weblog search request", e);
            throw new RuntimeException("Invalid search request format", e);
        }
    }
    ...
    private Map<String, Object> createRenderingModel(WeblogPageRequest pageRequest, WeblogSearchRequest searchRequest) {
        ...
        try {
            ...
            throw new RuntimeException("Error loading model objects for page", ex);
        }
        return model;
    }

    private Renderer getRenderer(Map<String, Object> model, WeblogSearchRequest searchRequest) {
        try {
            ThemeTemplate page = searchRequest.getWeblog().getTheme().getTemplateByAction(ThemeTemplate.ComponentType.SEARCH);
            if (page == null) {
                page = searchRequest.getWeblog().getTheme().getDefaultTemplate();
            }
            if (page == null) {
                throw new WebloggerException("Could not lookup default page for weblog " + searchRequest.getWeblogHandle());
            }
            return RendererManager.getRenderer(page, searchRequest.getDeviceType());
        } catch (Exception e) {
            log.error("Couldn't find renderer for rsd template", e);
            throw new RuntimeException("Couldn't find renderer for rsd template", e);
        }
    }
    ...
    private CachedContent renderContent(Map<String, Object> model, Renderer renderer) {
        ...
        try {
            ...
            throw new RuntimeException("Error during rendering for rsd template", e);
        }
        return rendererOutput;
    }

    private void flushRenderedContent(HttpServletResponse response, CachedContent rendererOutput) {
        try {
            response.setContentType("text/html; charset=utf-8");
            response.setContentLength(rendererOutput.getContent().length);
            response.getOutputStream().write(rendererOutput.getContent());
        } catch (IOException e) {
            log.error("Error flushing rendered content", e);
            throw new RuntimeException("Error flushing rendered content", e);
        }
    }
}